...
public class CodeBookGui extends ContainerScreen<CodeBookContainer> {
	...
	public void render(MatrixStack ms, int mouseX, int mouseY, float partialTicks) {
		...
		if (!tooltip.isEmpty()) {
			func_243308_b(ms, tooltip, mouseX, mouseY);
		} else {
	...
	public boolean charTyped(char typedChar, int keyCode) {
		...
		if (!Character.isISOControl(typedChar)) {
			...
			if (lines.get(line).length() < getMaxColumns()) {
				lines.get(line).insert(column, typedChar);
	...
	private int drawString(MatrixStack ms, String text, int x, int y, int color) {
		return font.drawString(ms, text, (float) x, (float) y, color);
	}

	private void drawRect(MatrixStack ms, int x, int y, int w, int h, int color) {
	...
	private int xToColumn(final int x, final int line) {
		return font.trimStringToWidth(ITextProperties.func_240652_a_(getLine(line).toString()), Math.max(0, x - CODE_POS_X)).size();
	}
	...
	private boolean isValidPaste(final String[] pastedLines) {
		...
		if (pastedLines.length - 1 + lines.size() > getMaxLinesPerPage())
			return false; // Invalid paste, too many resulting lines.
		if (pastedLines[0].length() + getLine(selectedLine).length() > getMaxColumns())
	...
	private void drawProgram(MatrixStack ms, final int mouseX, final int mouseY) {
		int position = 0;
		for (int lineNumber = 0; lineNumber < lines.size(); lineNumber++) {
			...
			if (selectionStart != selectionEnd && intersectsSelection(position, end)) {
				...
				drawString(ms, prefixText, currX, lineY, getCodeColor());
				...
				drawRect(ms, currX - 1, lineY - 1, currX + selectedWidth, lineY + font.FONT_HEIGHT - 1, getSelectionBackgroundColor());
				drawString(ms, selectedText, currX, lineY, getSelectionColor());
				currX += selectedWidth;
				final String postfixString = line.substring(prefix + selected);
				drawString(ms, postfixString, currX, lineY, getCodeColor());
			} else {
				// No selection here, just draw the line. Get it? "draw the line"?
				drawString(ms, line.toString(), lineX, lineY, getCodeColor());
			}
			position += line.length() + 1;
			// Instruction hints
			if (lineNumber <= instructionIds.size()) {
				...
				if (ip >= 0) {
					drawString(ms, String.format("%03X", (ip & 0x0fff)), CODE_POS_X + CODE_POS_X_IP_HINT_OFFSET, lineY + 1, getInstructionNoColor());
				}
			}
		}
		...
		if (compileError.size() > 0) {
			for (ParseException exception : compileError) {
				drawError(ms, exception, mouseX, mouseY);
			}
		}
	...
	private void drawError(MatrixStack ms, ParseException exception, final int mouseX, final int mouseY) {
	...
	private void drawTextCursor(MatrixStack ms) {
		if (System.currentTimeMillis() % 800 <= 400) {
			...
			drawRect(ms, x + 1, y + 1, x + 2 + 1, y + font.FONT_HEIGHT + 1, 0xCC333333);
			drawRect(ms, x, y, x + 2, y + font.FONT_HEIGHT, getSelectionColor());
		}
	}

	private void drawPageInfo(MatrixStack ms) {
		String pageInfo = String.format("%d/%d", data.getSelectedPage() + 1, data.getPageCount());
		drawString(ms, pageInfo, PAGE_NUMBER_X - (font.getStringWidth(pageInfo) / 2), PAGE_NUMBER_Y, getCodeColor());
	}
